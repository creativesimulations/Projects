...
namespace EpicBall
{
    public class ConsumerBlock : Block, ITeleportable
    {
        ...
        private void CheckConsume(Collision other)
        {
            if (GetClipPlayed())
            {
                SetClipPlayed(false);
        ...
        public void PlayParticle(Vector3 pointToSpawn, Vector3 size)
        {
            _particle = objectPooler.PopFromPool(objectPooler.consumeParticle, false, true, objectPooler.consumeParticleParent, false);
